...
def display_final_results(monthly_payments_var, loan_duration_var,
                          loan_amount_var)
  prompt "Calculating..."
  prompt "Your monthly payment amount is #{monthly_payments_var.round(2)}."
  prompt "Your total payment amount is #{(monthly_payments_var *
...
loop do
  ...
  while true
    ...
    if total_years + total_months > 0
      ...
      prompt "both years and months cannot be 0,
      please input a valid time frame"
    end
  end

  prompt "Total loan duration is #{total_years} years and
  #{total_months} months."
